...
+ Click *Edit security groups* link and edit/add the following rules
...
```
ubuntu@ip-my-ip:~$ git clone https://github.com/MikeHannon/myProject.git
```
...
```bash
ubuntu@ip-my-ip:~/myRepo$ virtualenv venv
ubuntu@ip-my-ip:~/myRepo$ source venv/bin/activate
ubuntu@ip-my-ip:~/myRepo$ pip install -r requirements.txt
ubuntu@ip-my-ip:~/myRepo$ pip install gunicorn
ubuntu@ip-my-ip:~/myRepo$ pip install psycopg2
```
...
```bash
ubuntu@ip-my-ip:~/myRepo$ cd myProject
ubuntu@ip-my-ip:~/myRepo/myProject$ sudo nano settings.py
...
```bash
(venv) ubuntu@ip-my-ip:~myRepo$ python manage.py collectstatic #say yes
(venv) ubuntu@ip-my-ip:~myRepo$ python manage.py makemigrations
(venv) ubuntu@ip-my-ip:~myRepo$ python manage.py migrate
```
Migrations made. Now let's run the following:

```bash
(venv) ubuntu@ip-my-ip:~myRepo$ gunicorn --bind 0.0.0.0:8000 projectName.wsgi:application
```
...
```bash
ubuntu@ip-my-ip:~$ sudo nano /etc/nginx/sites-available/myProject
```
...
```bash
ubuntu@ip-my-ip:~$ sudo ln -s /etc/nginx/sites-available/{{myProject}} /etc/nginx/sites-enabled
